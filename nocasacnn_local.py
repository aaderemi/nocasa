...
class CustomDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return (torch.tensor(mfcc).unsqueeze(0)/nnorm, score-1, file_idx)


trn_ds = CustomDataset(list(train_idxs), "train_normbest")
val_ds = CustomDataset(list(test_idxs), "train_normbest")
...
def main_training_loop(train_func, val_func, num_epochs, device, run_name, loss, model, bs, lr, accum_steps, useauto=False):
    ...
    for epoch in tqdm(range(num_epochs)):
      ...
      if val_results['loss'] < min_error:
        ...
        torch.save(best_model.state_dict(), f"models_local/model_{run_name}.pth")
...
bm = main_training_loop(training, validation, 30, device, "cnn_localn", loss, base_model, bs, lr, accum_steps, useauto=False)
